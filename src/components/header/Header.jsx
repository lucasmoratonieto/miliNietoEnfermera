      ...
      {/* Hamburguer */}
          ...
          <div className={`absolute lg:hidden z-20 right-4 ${smallHeader  ? 'absolute items-end ':'absolute '}`}>
            <button onClick={toggleHeader}  className='flex flex-col w-8 border-0 bg-transparent gap-[.5rem] rounded-sm mr-1 pt-7'>
              <div className={` bg-black h-[2px] w-[100%] rounded-sm transition-all duration-500 ${hamburguerCross ? '' : 'origin-left rotate-[40deg]'}`}> </div>
              <div className={` bg-black h-[2px] w-[100%] rounded-sm transition-all duration-500 ${hamburguerCross ? '' : 'origin-left w-0 opacity-0 '}`}> </div>
              <div className={` bg-black h-[2px] w-[100%] rounded-sm transition-all duration-500 ${hamburguerCross ? '' : 'origin-left -rotate-[40deg]'}`}> </div>

            </button>
          </div>

      <div id='nav-bar' className= {`flex transition-all duration-500 ease-in-out absolute flex-col gap-5 right-0 z-10 bg-slate-50 pt-[1.25rem] pb-16 px-5 w-56 h-60 rounded-3xl shadow-3xl lg:hidden   ${smallHeader ? '':`absolute right-0 opacity-0  }  `} `}>
        {/* header on small screen */}
            <div className={`flex-col px-10 transition-all duration-500 ease-in-out  ${smallHeader ? ' ' : ' opacity- h-0 text-[0]'}`}>
